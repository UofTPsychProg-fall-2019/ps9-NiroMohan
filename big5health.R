...
gather('A_1','A_2','A_3','A_4','A_5','A_6','A_7','A_8','A_9','A_10',
      ...
      'O_1', 'O_10',key=traititems, value=score)
...
ipip.l <- ipip.l %>% 
  separate(traititems,into=c('trait','items'),sep="_")

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% 
  group_by(RID,age,gender, exer, BMI,trait,) %>% summarize(avgtrait=mean(score))
...
#####This was completed in the previous step when calculating the averages.### 
...
exer.avg <- ipip.comp %>% 
  group_by(trait,exer) %>% summarize(avgexer=mean(avgtrait),semexer=sd(avgtrait)/sqrt(5000))
...
ggplot(exer.avg,aes(x=trait,y=avgexer,colour=exer))+
  geom_pointrange(aes(ymin=avgexer-semexer,ymax=avgexer+semexer),
               position=dodge)+
...
gender.avg <- ipip.comp %>% 
  group_by(trait,gender) %>% summarize(avggender=mean(avgtrait),semgender=sd(avgtrait)/sqrt(5000))

# create a gender plot and compare to the answer figure:
ggplot(gender.avg,aes(x=trait,y=avggender,colour=gender))+
  geom_pointrange(aes(ymin=avggender-semgender,ymax=avggender+semgender),
                  position=dodge)+
...
age.avg <- ipip.comp %>% 
  group_by(trait)%>%
  summarise(corrcoef = cor(age, avgtrait))
